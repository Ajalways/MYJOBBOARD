...
import { generateChallenge, scoreSubmission, isOpenAIConfigured } from '../services/openaiService.js';

const router = express.Router();
const prisma = new PrismaClient();

// 1. POST /api/challenges/generate - Generate AI-powered challenge
router.post('/generate', authenticateToken, async (req, res) => {
  try {
    const { job_id, difficulty, topic } = req.body;

    if (!job_id || !difficulty || !topic) {
    ...
    const challenge = await prisma.challenge.create({
      data: {
        ...
        answer_key: aiChallenge.answer_key,
        scoring_rubric: aiChallenge.scoring_rubric
      },
      include: {
...
// 2. POST /api/challenges/submit - Submit and score challenge with AI
router.post('/submit', authenticateToken, async (req, res) => {
  try {
    const { challenge_id, application_id, submission } = req.body;

    if (!challenge_id || !application_id || !submission) {
      return res.status(400).json({ 
        error: 'Missing required fields: challenge_id, application_id, submission' 
      });
    }
    ...
    let score = null;
    let feedback = 'Submitted successfully. Awaiting manual review.';

    // Score with AI if OpenAI is configured
    if (isOpenAIConfigured()) {
      try {
        const aiScore = await scoreSubmission(
          challenge.prompt,
...
// 3. GET /api/challenges/:job_id - Get all challenges for a job
router.get('/:job_id', authenticateToken, async (req, res) => {
  try {
    ...
    // Allow access for job owner, admin, or candidates who applied
    const hasAccess = job.created_by === req.user.id || 
                     req.user.role === 'ADMIN' ||
    ...
    const challenges = await prisma.challenge.findMany({
      ...
      select: {
        ...
        prompt: true,
        created_at: true,
        // Only include answer_key for job owners/admins
        ...(job.created_by === req.user.id || req.user.role === 'ADMIN' ? {
          answer_key: true,
          scoring_rubric: true,
          results: {
            include: {
...
// Get all challenges for a job (for companies) - LEGACY ROUTE
...
// Test OpenAI configuration endpoint
router.get('/ai-status', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      ...
      message: configured 
        ? 'OpenAI is properly configured and ready for challenge generation'
        : 'OpenAI API key not set. Please configure OPENAI_API_KEY environment variable'
    });
  } catch (error) {
